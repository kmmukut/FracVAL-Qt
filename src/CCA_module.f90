...
subroutine CCA_sub(not_able_cca,not_able_pca,save_output)
use Ctes
use PCA_Subclusters_module
use Save_results_CC
implicit none
logical, intent(inout) :: not_able_cca, not_able_pca
logical, intent(in), optional :: save_output
logical :: do_save_output
...
not_able_pca = .false.
do_save_output = .true.
if (present(save_output)) do_save_output = save_output
...
if (not_able_cca .OR. not_able_pca) then
    ...
    if (do_save_output) call Save_results(X,Y,Z,R,N,iter,output_dir,contact_overlaps,contact_count)
end if
...
call CCA_Sticking_process_v1(COR1,CM1,n1,COR2,CM2,n2,theta_a,vec_0,i_vec,j_vec,prev_cand1,prev_cand2,Gamma_pc,Gamma_real,Ext_case,contact_overlap)
    !CCA_Sticking_process_v1(COR1,CM1,n1,COR2,CM2,n2,theta_a,vec_0,i_vec,j_vec,prev_cand1,prev_cand2,Gamma_pc,Gamma_real,Ext_case)
        ...
        ! Overlaping check
         call CCA_Overlapping_check(Cov_max, X1,Y1,Z1,R1,n1, X2,Y2,Z2,R2,n2,prev_cand1,prev_cand2,contact_overlap)
...
subroutine CCA_Sticking_process_v1(COR1,CM1,n1,COR2,CM2,n2,theta_a,vec_0,i_vec,j_vec,prev_cand1,&
                                   prev_cand2,Gamma_pc,Gamma_real,Ext_case,contact_overlap)
!use Save_results_CC
implicit none
integer, intent(in) :: n1, n2, Ext_case
integer, intent(in) :: prev_cand1, prev_cand2
real, intent(inout) ::  COR1(n1,4), COR2(n2,4)
real, intent(in) ::  Gamma_pc, contact_overlap
logical, intent(in) :: Gamma_real
...
r1_sph1 = (R1(prev_cand1) + R2(prev_cand2))*(1.0-contact_overlap)
...
subroutine CCA_Overlapping_check(Cov_max, X1,Y1,Z1,R1,n1, X2,Y2,Z2,R2,n2,contact1,contact2,contact_overlap)
implicit none
integer, intent(in) :: n1, n2, contact1, contact2
real, intent(in) :: X1(n1),Y1(n1),Z1(n1),R1(n1), X2(n2),Y2(n2),Z2(n2),R2(n2)
real, intent(in) :: contact_overlap
real, intent(out) :: Cov_max
real Dij, Cij, effective_overlap
integer i,j
...
do i = 1,n1
    do j = 1,n2
        ...
        if (Dij .LT. (R1(i)+R2(j))) then
            ...
            if (i == contact1 .and. j == contact2) then
                effective_overlap = max(0.0, Cij-contact_overlap)
            else
                effective_overlap = Cij
            end if
            if (effective_overlap .GT. Cov_max) Cov_max = effective_overlap
        end if
    end do
end do
