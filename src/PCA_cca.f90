...
use Ctes, only: sample_contact_overlap, record_contact_overlap
implicit none
...
! Sample one intended contact overlap for this newly attached monomer.
! Candidate changes/rotations keep the same target so the distribution is per contact.
contact_overlap = sample_contact_overlap()
...
! Overlaping check
call Overlapping_check(Cov_max, X(1:k),Y(1:k),Z(1:k),R(1:k),k,selected_real,contact_overlap)
...
call record_contact_overlap(contact_overlap)
...
real theta, phi, contact_overlap, contact_distance
...
contact_overlap = sample_contact_overlap()
contact_distance = (R(1)+R(2))*(1.0-contact_overlap)
X(2) = X(1) + contact_distance*cos(theta)*sin(phi)
Y(2) = Y(1) + contact_distance*sin(theta)*sin(phi)
Z(2) = Z(1) + contact_distance*cos(phi)
call record_contact_overlap(contact_overlap)
...
Y1 = Y_sel
Z1 = Z_sel
R1 = (R_sel + R_k)*(1.0-contact_overlap)
...
subroutine Overlapping_check(Cov_max, X_con, Y_con, Z_con, R_con,k,contact_index,contact_overlap)
implicit none
integer, intent(in) :: k, contact_index
real, intent(in) :: X_con(k), Y_con(k), Z_con(k), R_con(k)
real, intent(in) :: contact_overlap
real, intent(out) :: Cov_max
real C(k-1)
real distanc_kj, raw_overlap
integer jj

! Check the new monomer against the existing aggregate. The selected contact
! is allowed its sampled/fixed intended overlap; all other pairs retain the
! strict numerical tolerance applied by the caller.
do jj = 1,(k-1)
    distanc_kj = sqrt((x_con(k)-x_con(jj))**2. + (y_con(k)-y_con(jj))**2. + (z_con(k)-z_con(jj))**2.)

    if (distanc_kj .LT. (R_con(k)+R_con(jj))) then
        raw_overlap = ((R_con(k)+R_con(jj))-distanc_kj)/(R_con(k)+R_con(jj))
        if (jj == contact_index) then
            C(jj) = max(0.0, raw_overlap-contact_overlap)
        else
            C(jj) = raw_overlap
        end if
    else
