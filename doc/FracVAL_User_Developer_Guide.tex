...
\hypersetup{
  ...
  urlcolor=FracTeal,
  citecolor=FracBlue,
  pdftitle={FracVAL-Qt 1.0.1 User and Developer Guide},
  pdfauthor={FracVAL-Qt project documentation}
}

\pagestyle{fancy}
\fancyhf{}
\lhead{FracVAL-Qt 1.0.1}
...
\begin{titlepage}
  ...
  {\Huge\bfseries FracVAL-Qt 1.0.1\\[0.25cm]}
  ...
  {\small Documentation release: 1.0.1}
\end{titlepage}
...
\begin{itemize}
  ...
  \item GNU Make on macOS/Linux for the optional Makefile shortcuts; Windows uses the Python build script directly.
\end{itemize}
...
\code{python tools/build.py all} & Build executable and extension on any platform. \\
\code{make python-ext PYTHON=python} & Rebuild only the F2PY extension (POSIX shortcut for \code{tools/build.py ext}). \\
\code{python tools/build.py clean} & Remove native build products. \\
...
\code{QApplication} to avoid the common empty-plugin-path failure. On Windows
the expected plugin is \code{windows}.
...
\code{tests/python/test\_fortran\_cli.py} replaced the earlier shell harness
(\code{tests/run\_tests.sh}), so the whole suite runs under \code{pytest} on
every platform.
...
constructs the application without requiring a visible display. It is opt-in
and is not collected by a plain \code{python -m pytest} run.
...
\code{fracval-info} reports the discovered Fortran and C compilers, so a
missing or mismatched toolchain shows up directly. Then clean and rebuild:
...
\code{python tools/build.py} & Cross-platform build script (\code{exe}, \code{ext}, \code{all}, \code{clean}). \\
\bottomrule
\end{longtable}
...
\code{make test} & Run the pytest suite. \\
...
\code{make clean} & Remove native build artifacts (\code{tools/build.py clean}). \\
\code{python tools/build.py all} & Build executable and extension on any platform. \\
\bottomrule
\end{longtable}
